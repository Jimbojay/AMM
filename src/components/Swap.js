...
const Swap = () => {
  ...
  const swapHandler = async (e) => {
    ...
    const _inputAmout = ethers.utils.parseUnits(inputAmount, 'ether')

    // Swap token depending upon which on wer're doing...
    if (inputToken ==="DAPP") { 
      await swap(provider,amm, tokens[0], inputToken, _inputAmout, dispatch)
    } else {
      await swap(provider,amm, tokens[1], inputToken, _inputAmout, dispatch)
    }
  ...
  useEffect(() => { //if amm is loaded, 
    if(inputToken && outputToken) {
      getPrice()      
    }
  }, [inputToken, outputToken]); //if amm is canged
